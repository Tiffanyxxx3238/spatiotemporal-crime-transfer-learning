...
LA_API_NEW = "https://data.lacity.org/resource/2nrs-mtv8.json"   # 2020–present
LA_API_OLD = "https://data.lacity.org/resource/63jg-8b9z.json"   # 2010–2019
...
def download_la():
    ...
    # 2010–2019 (older dataset)
    print("[LA] Downloading 2010-2019 (older dataset)...")
    ...
    # 2020–2024 (newer dataset)
    print("[LA] Downloading 2020-2024 (newer dataset)...")
    for year in range(2020, 2025):
        ...
        try:
            r = requests.get(LA_API_NEW, params=params, timeout=120)
            r.raise_for_status()
            df_yr = pd.DataFrame(r.json())
            print(f"{len(df_yr):,}")
            all_dfs.append(df_yr)
    ...
    if all_dfs:
        df = pd.concat(all_dfs, ignore_index=True)
        df.to_csv(out_path, index=False)
        print(f"[LA] Done: {len(df):,} records (2010-2024) -> {out_path}")
