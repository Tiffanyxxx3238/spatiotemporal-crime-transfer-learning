...
PROC_DIR  = _os.path.join(_BASE, '..', 'data', 'processed')
MODEL_DIR = _os.path.join(_BASE, '..', 'outputs', 'models')
EDA_DIR   = _os.path.join(_BASE, '..', 'outputs', 'eda')
...
# ============================================================
# CELL 1: Load data — read Karachi directly from karachi_clean.csv
# ============================================================
import os as _os2
_kar_path = _os2.path.abspath(_os2.path.join(PROC_DIR, 'karachi_clean.csv'))
print(f"Reading Karachi data from: {_kar_path}")
df = pd.read_csv(_kar_path, low_memory=False)
# Map drug/public_order → other before filtering
MERGE_MAP = {'drug': 'other', 'public_order': 'other'}
df['crime_category'] = df['crime_category'].replace(MERGE_MAP)
...
GRID_SIZE = 0.005 if CITY == 'Karachi' else 0.01
...
MAP_DIR = _os.path.join(_BASE, '..', 'outputs', 'maps')
...
_nyc_path = _os2.path.abspath(_os2.path.join(PROC_DIR, 'nyc_clean.csv'))
city_events = pd.read_csv(_nyc_path, low_memory=False)
city_events['datetime'] = pd.to_datetime(city_events['datetime'], errors='coerce')
city_events = city_events.dropna(subset=['datetime'])
city_events['crime_category'] = city_events['crime_category'].replace({'drug': 'other', 'public_order': 'other'})
city_events = city_events[city_events['crime_category'].isin(CATEGORIES)]
...
# Reload Chicago data for ablation study
CITY = 'Chicago'
GRID_SIZE = 0.01

_chi_path = _os2.path.abspath(_os2.path.join(PROC_DIR, 'chicago_clean.csv'))
city_events = pd.read_csv(_chi_path, low_memory=False)
city_events['datetime'] = pd.to_datetime(city_events['datetime'], errors='coerce')
city_events = city_events.dropna(subset=['datetime'])
city_events['crime_category'] = city_events['crime_category'].replace({'drug': 'other', 'public_order': 'other'})
city_events = city_events[city_events['crime_category'].isin(CATEGORIES)]
...
_nyc_path2 = _os2.path.abspath(_os2.path.join(PROC_DIR, 'nyc_clean.csv'))
city_events = pd.read_csv(_nyc_path2, low_memory=False)
city_events['datetime'] = pd.to_datetime(city_events['datetime'], errors='coerce')
city_events = city_events.dropna(subset=['datetime'])
city_events['crime_category'] = city_events['crime_category'].replace({'drug': 'other', 'public_order': 'other'})
city_events = city_events[city_events['crime_category'].isin(CATEGORIES)]
