"""
Crime Classification — Peoria, IL
Data: 2023-2026 (~67k records, 3 years)
Temporal split: 60/20/20 (short window, same as SLC)
Grid size: 0.01° (~1km²)
"""
import pandas as pd
import numpy as np
...
df = pd.read_csv(f'{PROC}/peoria_clean.csv', low_memory=False)
...
def add_spatial_lag(g, k=4):
    ...
    for i, pt in enumerate(coords):
        ...
        for c in CATEGORIES:
            col = f'hist_{c}'
            lags[f'lag_{c}'].append(g[col].iloc[neighbors].mean())
...
train_g = add_pct(train_g, train_df)
val_g   = add_pct(val_g,   train_df)
test_g  = add_pct(test_g,  train_df)
...
# ── CatBoost ──────────────────────────────────────────────────────────────────
print('\nTraining CatBoost...')
weights = {i: len(y_train)/(len(CATEGORIES)*np.bincount(y_train)[i]+1) for i in range(len(CATEGORIES))}
sample_w = np.array([weights[y] for y in y_train])
...
proba_cb_val  = pad_proba(cb_model.predict_proba(X_val),  len(CATEGORIES))
proba_lgb_val = pad_proba(lgb_model.predict_proba(X_val), len(CATEGORIES))
proba_val_ens = (proba_cb_val + proba_lgb_val) / 2

iso_list = []
for c in range(len(CATEGORIES)):
    iso = IsotonicRegression(out_of_bounds='clip')
    iso.fit(proba_val_ens[:, c], (y_val == c).astype(int))
    iso_list.append(iso)
joblib.dump(iso_list, f'{MODEL_DIR}/cal_iso_{CITY_KEY}.pkl')
joblib.dump(le,       f'{MODEL_DIR}/label_encoder_{CITY_KEY}.pkl')

# ── Test evaluation ───────────────────────────────────────────────────────────
proba_cb_test  = pad_proba(cb_model.predict_proba(X_test),  len(CATEGORIES))
proba_lgb_test = pad_proba(lgb_model.predict_proba(X_test), len(CATEGORIES))
proba_test_ens = (proba_cb_test + proba_lgb_test) / 2
proba_test_cal = np.stack([iso_list[c].predict(proba_test_ens[:, c])
                           for c in range(len(CATEGORIES))], axis=1)
proba_test_cal = proba_test_cal / proba_test_cal.sum(axis=1, keepdims=True).clip(1e-9)

y_pred = proba_test_cal.argmax(axis=1)
...
# ── Transfer learning (zero-shot NYC → Peoria) ────────────────────────────────
print('\n--- Transfer Learning: Zero-shot NYC → Peoria ---')
try:
    nyc_model = lgb.Booster(model_file=f'{MODEL_DIR}/model_nyc_lgb.pkl')
    p_nyc = nyc_model.predict(X_test.values)
    if p_nyc.ndim == 1: p_nyc = np.eye(len(CATEGORIES))[p_nyc.astype(int)]
    y_zs = p_nyc.argmax(axis=1)
    prec_zs = precision_score(y_test, y_zs, average='macro', zero_division=0)
    print(f'Zero-shot NYC→Peoria Precision: {prec_zs:.4f}')
