#!/usr/bin/env python
# coding: utf-8
# Cross-City Crime Pattern Analysis & Transfer Learning — Detroit
...
# CELL 1: Load data
# ============================================================
_clean_path = os.path.join(PROC_DIR, 'detroit_clean.csv')
...
def add_spatial_lag(grid, k=8):
    ...
    dists, idxs = tree.query(np.radians(coords), k=k+1)
    for cat in CATEGORIES:
...
# Risk = confidence * count (normalised to 0-100)
...
# ============================================================
methods = [
    {'Method': 'CatBoost (no threshold)',
     'Precision Macro': round(pm_cb, 4), 'F1 Macro': round(f1_cb, 4), 'Coverage': 1.0},
    {'Method': 'LightGBM (no threshold)',
     'Precision Macro': round(pm_lgb, 4), 'F1 Macro': round(f1_lgb, 4), 'Coverage': 1.0},
]
...
# ============================================================
# CELL 9: Transfer learning — NYC → Detroit
# ============================================================
try:
    ...
    # Align class order: NYC model → Detroit le order
    nyc_order = list(nyc_le.classes_)
    det_order = list(le.classes_)
    idx_map   = [nyc_order.index(c) for c in det_order if c in nyc_order]
    ...
    transfer_df.to_csv(f'{MODEL_DIR}/transfer_nyc_{CITY.lower()}.csv', index=False)
    print(f'\n===== Transfer NYC → {CITY} =====')
