...
# Skip XGBoost/RF if fewer than 3 classes in train
if len(set(y_train)) >= 3:
    ...
    y_val_xgb   = y_val   - y_train.min()
    model_xgb.fit(
        X_train.values, y_train_xgb,
        sample_weight=sample_weights_xgb,
        eval_set=[(X_val.values, y_val_xgb)],
        verbose=False,
    )
    ...
    model_xgb = None
    model_rf  = None
    print('Skipping XGBoost/RF: DC has fewer than 3 classes')
...
pm_xgb, pw_xgb, f1_xgb, proba_xgb = (0,0,0,None) if model_xgb is None else full_eval(model_xgb, X_test, y_test, le, 'XGBoost')[1:]
pm_rf, pw_rf, f1_rf, proba_rf = (0,0,0,None) if model_rf is None else full_eval(model_rf, X_test, y_test, le, 'RandomForest')[1:]
...
proba_xgb_val = model_xgb.predict_proba(X_val.values) if model_xgb is not None else None
proba_rf_val  = model_rf.predict_proba(X_val.values)  if model_rf  is not None else None
...
sweep_results = selective_precision_sweep([
    ...
    *([('XGBoost raw', proba_xgb)] if proba_xgb is not None else []),
    *([('RandomForest', proba_rf)]  if proba_rf  is not None else []),
], y_test)
...
for i, cat in enumerate(le.classes_):
    risk_df[f'proba_{cat}'] = proba_all_cal[:,i].round(4) if i < proba_all_cal.shape[1] else 0.0
risk_df['risk_score'] = (proba_all_cal[:,violent_idx].round(1) * 100) if violent_idx < proba_all_cal.shape[1] else 0.0
risk_df['city']       = CITY
...
def save_city_outputs(CITY, model_cat, model_lgb,
    ...
    for i, cat in enumerate(le.classes_):
        risk_df[f'proba_{cat}'] = proba_cal[:, i].round(4) if i < proba_cal.shape[1] else 0.0

    risk_df['risk_score'] = (proba_cal[:, violent_idx] * 100).round(1) if violent_idx < proba_cal.shape[1] else 0.0
    risk_df['city']       = CITY
