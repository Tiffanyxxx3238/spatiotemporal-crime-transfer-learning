#!/usr/bin/env python
# coding: utf-8
# Cross-City Crime Pattern Analysis — Seattle
...
# ── Load ─────────────────────────────────────────────────────────────────────
df = pd.read_csv(os.path.join(PROC_DIR, 'seattle_clean.csv'), low_memory=False)
...
cb.save_model(os.path.join(MODEL_DIR, f'model_{CITY.lower()}_catboost.cbm'))
