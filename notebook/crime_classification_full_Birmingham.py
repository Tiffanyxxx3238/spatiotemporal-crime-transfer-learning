#!/usr/bin/env python
# coding: utf-8
# Cross-City Crime Pattern Analysis & Transfer Learning — Birmingham, Alabama
# NOTE: Birmingham open data only covers Part I UCR crimes (assault, robbery, theft,
# burglary, rape). Drug/public_order offenses are absent from the portal.
# Categories are effectively binary: property vs violent (with a tiny 'other' slice).
...
# CELL 1: Load data
# ============================================================
_clean_path = os.path.join(PROC_DIR, 'birmingham_clean.csv')
...
# ============================================================
# CELL 3: Temporal split (60/20/20)
# ============================================================
city_events = df.copy()
...
def add_spatial_lag(grid, k=8):
    ...
    dists, idxs = tree.query(np.radians(coords), k=k_eff)
    for cat in CATEGORIES:
        vals = grid[f'hist_{cat}'].values
        grid[f'lag_{cat}'] = np.array([vals[idx[1:]].mean() if len(idx) > 1 else vals[i]
...
cb.save_model(f'{MODEL_DIR}/model_{CITY_KEY}_catboost.cbm')
...
plt.tight_layout()
plt.savefig(f'{EDA_DIR}/cm_{CITY_KEY}_catboost.png', dpi=120)
plt.close()
...
plt.tight_layout()
plt.savefig(f'{EDA_DIR}/feature_importance_{CITY_KEY}.png', dpi=120)
plt.close()

# ============================================================
# CELL 6: Calibration
# ============================================================
proba_cb  = cb.predict_proba(prep_cat(X_val))
proba_lgb = lgb_model.predict_proba(X_val)

# CatBoost may collapse to fewer classes when a class never dominates any grid.
# Pad to full CATEGORIES width so downstream code is consistent.
...
proba_cb_full  = pad_proba(proba_cb,  len(CATEGORIES))
proba_lgb_full = pad_proba(proba_lgb, len(CATEGORIES))

cal_platt = LogisticRegression(C=1.0, max_iter=1000, multi_class='multinomial')
cal_platt.fit(proba_cb_full, y_val)
joblib.dump(cal_platt, f'{MODEL_DIR}/cal_platt_{CITY_KEY}.pkl')

cal_iso_list = []
for c in range(len(CATEGORIES)):
    iso = IsotonicRegression(out_of_bounds='clip')
    iso.fit(proba_cb_full[:, c], (y_val == c).astype(int))
    cal_iso_list.append(iso)
joblib.dump(cal_iso_list, f'{MODEL_DIR}/cal_iso_{CITY_KEY}.pkl')

joblib.dump(le, f'{MODEL_DIR}/label_encoder_{CITY_KEY}.pkl')
print('Calibration complete, models saved.')

# ============================================================
# CELL 7: Grid risk scores
# ============================================================
proba_test_lgb_raw = lgb_model.predict_proba(X_test)
proba_test_lgb = pad_proba(proba_test_lgb_raw, len(CATEGORIES))
...
risk_out = f'{MODEL_DIR}/grid_risk_{CITY_KEY}.csv'
risk_df.to_csv(risk_out, index=False)
acc = risk_df['ok'].mean()
print(f'[{CITY}] Saved: {risk_out}  ({len(risk_df):,} grids)')
...
cmp_df = pd.DataFrame(methods)
cmp_df.to_csv(f'{MODEL_DIR}/method_comparison_{CITY_KEY}.csv', index=False)
print(f'\n===== Method Comparison [{CITY}] =====')
print(cmp_df.to_string(index=False))

# ============================================================
# CELL 9: Transfer learning — NYC → Birmingham
# ============================================================
try:
    nyc_cb  = CatBoostClassifier()
    nyc_cb.load_model(f'{MODEL_DIR}/model_nyc_catboost.cbm')
    nyc_lgb = joblib.load(f'{MODEL_DIR}/model_nyc_lgb.pkl')
    nyc_le  = joblib.load(f'{MODEL_DIR}/label_encoder_nyc.pkl')

    nyc_order = list(nyc_le.classes_)
    brm_order = list(le.classes_)
    idx_map   = [nyc_order.index(c) for c in brm_order if c in nyc_order]

    proba_zs = nyc_cb.predict_proba(prep_cat(X_test))[:, idx_map]
    ...
    T = 3.0
    nyc_proba_train = nyc_cb.predict_proba(prep_cat(X_train))[:, idx_map]
    soft_labels = np.exp(np.log(nyc_proba_train + 1e-9) / T)
    ...
    transfer_df = pd.DataFrame([
        {'Scenario': f'{CITY} baseline (LightGBM)', 'Precision Macro': round(pm_lgb, 4), 'F1 Macro': round(f1_lgb, 4)},
        {'Scenario': f'Zero-shot NYC->{CITY}',       'Precision Macro': round(pm_zs, 4),  'F1 Macro': round(f1_zs, 4)},
        {'Scenario': 'Teacher-Student (T=3)',         'Precision Macro': round(pm_ts, 4),  'F1 Macro': round(f1_ts, 4)},
    ])
    transfer_df.to_csv(f'{MODEL_DIR}/transfer_nyc_{CITY_KEY}.csv', index=False)
    print(f'\n===== Transfer NYC → {CITY} =====')
